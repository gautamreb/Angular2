    ...
    $(document).ready(function () {
       ...
       // $('#titleHeader').html(loggedInUserName);
    ...
    function changeTabs(name) {
        ...
        for (var i = 0; i < tabsLiArray.length; i++) {
         ...
         $.ajax({
             //url: baseURL + "account/mnogenericanalytics/overview?fromDate="+fromDate+"&toDate="+toDate+"&filterType="+ filtvar +"&mnoId=465&sortType="+ sortType +"&graphType="+sortType,
            //url: "http://localhost:8084/sawridgewebservices-mno/account/mnogenericanalytics/overview?fromDate="+fromDate+"&toDate="+toDate+"&filterType="+ filtvar +"&mnoId=465&sortType="+ sortType +"&graphType="+sortType,
            dataType: 'json',
            method: 'get',
            crossDomain: true,
            ...
            success: function (response) {
                ...
                totReq=response.map.TOTREQ;
                $("#totReqId").html(totReq);
                totSucess=response.map.TOTSUCCESS;
                $("#totSuccessId").html(totSucess + " %");
                dataVolume=response.map.DATAVOLUME;
                ...
                for(var i=1; i<len; i++)  
                {
                    ...
                    for(var j=0 ; j < response.Payload[i].length; j++)
                    {
                        ...
                        if(j===0)
                        {
                            console.log("0th position"+response.Payload[i][j].toString());
        ...
        $('#example').DataTable({
            ...
            'columnDefs': [{
                    ...
                    'render': function (data, type, full, meta) {
                        return '<input type="checkbox" name="case[]" id="checkId" class="ok" onclick=\"collectSelectedId(this)\" value="' + $('<div/>').text(data).html() + '">';
                    }
                }],
            order: [[1, 'asc']],
            "bDestroy": true,
    ...
    function addmenu(data, id) {
        ...
        var item = '<div class="row menuButton">' +
                '<div class="col-sm-12" style="text-align: center;" id="sideRow_' + data.id + '" onclick="clickofAnalytics(' + data.id + ');">' +
                '<img src="../img/'+ data.icon +'"'+ 'alt="" /><br/>' +
        ...
        $("#sideRow_2").css({"background-color": "#2C4887"})
    ...
    function collectSelectedId(obj) {
        ...
        $.each($("input[name='case[]']:checked"), function (index, value) {
            ...
            if (index >= 0)
            {
                testArry.push($(data).find('td:eq(0)').text());
                cnt++;
                if (cnt> 5 ){
    ...
    $('#btn').click(function () {
        $.ajax({
            ...
            //url: "http://localhost:8084/sawridgewebservices-mno/account/mnogenericanalytics/overview?fromDate="+fromDate+"&toDate="+toDate+"&filterType="+ filtvar +"&mnoId=465&sortType="+ sortType +"&graphType="+sortType+"&checked="+testArry,
            dataType: 'json',
            method: 'get',
            crossDomain: true,
            contentType: 'application/json',
