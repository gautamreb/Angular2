...
//				},
				dates: function() {
					...
					var today = new Date(); // customize
					dates[0] = new Date(today).setDate(today.getDate() - (days * 7)).valueOf();
...
//						'</div>'+
						'<div class="main-daterange">'+
							'<div>'+
								...
								'</span>'+
							'</div>'+
							'<input type="text" class="dr dr1 from" lastSel="0" placeholder="Start Date"/>   <input type="text" class="dr dr1 to" lastSel="1" placeholder="End Date"/>'+
...
//           			'</div>'+
						...
						'<button class="btn btn-mini btn btn-primary" id="button-ok">Apply</button>'+
			...
			$target.addClass('DRWClosed');
                        ...
                        //on load setting lastdays(selecting last 7 days) by default

                        internal.populateAggregations($target.data("DateRangesWidget").options.aggregations);
                        internal.populateDateRangePresets();
                        internal.loadValues($target);
                        internal.setDaterangePreset('lastdays');

                        internal.retractDropdown($target);
                        internal.saveValues($target);
                        internal.updateDateField($target);
		},
